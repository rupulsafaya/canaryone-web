...
The homepage no longer carries a comparison table or the install terminal directly; those live at `/evals`. The early-access waitlist submits to a Supabase `waitlist` table (`ghkebzdbexoqxuzneayk`, us-east-1) using the anon key with INSERT-only RLS — no credentials are exposed to the browser.
